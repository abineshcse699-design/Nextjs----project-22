// app/services/servicenow/capabilities/[slug]/page.tsx
//
// Built from the Software & Product Engineering capability page
// (hero -> overview -> capability cards -> dark section -> gradient
// carousel -> FAQ -> closing CTA), same tokens, type scale,
...
/* ===============================================================
   ROUTES — must match the real folders on disk (lowercase)
================================================================ */

const SERVICES_BASE = "/services";
const SERVICE_BASE = "/services/servicenow";
const CAPABILITY_BASE = "/services/servicenow/capabilities";
...
const processSteps = [
  {
    icon: Search,
    title: "Assess",
    body: "We review your current workflows, data, and licensing to understand where the platform can create the most value before anything is configured.",
  },
  {
    icon: Settings,
    title: "Configure",
    body: "Certified ServiceNow engineers configure the capability around how your teams actually work, keeping customization to what is truly needed.",
  },
  {
    icon: Plug,
    title: "Integrate",
    body: "We connect the capability to your identity, monitoring, and business systems, then support training and hypercare through go live.",
  },
  {
    icon: LifeBuoy,
    title: "Sustain",
    body: "After launch we tune workflows, plan release upgrades, and provide managed support from a team that knows your instance.",
  },
];
...
export async function generateMetadata({
  ...
  return {
    title: `${capability.title} | ServiceNow ITSM | Starfii`,
...
export default async function ServiceNowCapabilityDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  // Everything else in the ServiceNow ITSM grid.
        ...
        <Reveal as="section" className="mb-20 mt-16 lg:mb-24 lg:mt-20">
          <div className="group grid grid-cols-1 items-stretch overflow-hidden rounded-lg bg-[#F5F3FC] transition-colors duration-500 ease-out hover:bg-[#EAE4FA] lg:grid-cols-2">
            <div className="flex flex-col justify-center p-10 transition-transform duration-500 ease-out group-hover:translate-x-2 lg:p-14">
              ...
              >
                What Is ServiceNow {capability.title}?
              </h2>
          ...
          <div className="grid grid-cols-1 items-start gap-10 lg:grid-cols-[380px_1fr] lg:gap-14 xl:grid-cols-[420px_1fr]">
            <Reveal className="self-start lg:sticky lg:top-28">
              ...
              <p className="font-body mt-5 max-w-md text-[15px] leading-relaxed text-slate-600 sm:text-[16px]">
                ...
                ServiceNow {capability.title.toLowerCase()}, so your team can
                run IT service with confidence.
              </p>
            </Reveal>
