// Save as: app/services/servicenow/casestudies/[slug]/page.tsx
...
/* ---------------------------------------------------------------
   ROUTES
   The ServiceNow page has no /casestudies listing page yet, so the
   breadcrumb goes to the case study carousel on the ServiceNow page.
   Once you add casestudies/page.tsx, change LIST_HREF to
   "/services/servicenow/casestudies".
---------------------------------------------------------------- */
const SERVICENOW_PATH = "/services/servicenow";
const LIST_HREF = "/services/servicenow#case-studies";
...
export async function generateMetadata({
  ...
  return {
    title: `${study.title} | Starfii ServiceNow Case Study`,
...
function Breadcrumb({ title }: { title: string }) {
  return (
    ...
    >
      ...
      <ChevronRight className="h-3.5 w-3.5" />
      <Link href={SERVICENOW_PATH} className="hover:text-[#3a3ff0]">
        ServiceNow
      </Link>
      <ChevronRight className="h-3.5 w-3.5" />
...
export default async function ServiceNowCaseStudyDetailPage({
  params,
}: PageProps) {
  const { slug } = await params;
  const study = getCaseStudyBySlug(slug);
          ...
          <div className="min-w-0">
            ...
            <div className="mt-10">
              ...
              <section id="client" className="scroll-mt-28">
                ...
                {/* Client names here are long sentences, so the tile text is smaller */}
                <div className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-3">
                  {[
                    ...
                    <div key={tile.label}>
                      <p className="font-heading text-[22px] font-medium leading-snug text-[#3a3ff0] lg:text-[24px]">
                        {tile.value}
                      </p>
