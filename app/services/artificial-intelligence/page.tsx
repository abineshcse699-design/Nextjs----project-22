// app/services/artificial-intelligence/page.tsx
"use client";

import {
  ArrowUpRight,
  ChevronRight,
  BrainCircuit,
  Sparkles,
  Eye,
  MessagesSquare,
  LineChart,
  ShieldCheck,
  CheckCircle2,
} from "lucide-react";
import { motion, type Variants } from "framer-motion";

const T = {
  ink: "text-[#14163B]",
  inkBg: "bg-[#0C0E2A]",
  primary: "text-[#3B2FE0]",
  primaryBg: "bg-[#3B2FE0]",
  primaryHoverBg: "hover:bg-[#2E24B8]",
  muted: "text-[#5B5D78]",
  border: "border-[#E4E4EF]",
  panelBg: "bg-[#FAFAFD]",
};

// ✅ Matches navbar's outer wrapper exactly: max-w-[1830px] px-4 sm:px-6 md:px-8 lg:px-10 xl:px-12
const ALIGN = "mx-auto max-w-[1520px] px-6 sm:px-10 lg:px-16";

// --- Shared animation variants ---
// heroContainer / heroItem: play once on page load (hero only)
// container / item: play once, triggered on scroll into view (rest of page)
const heroContainer: Variants = {
  hidden: {},
  visible: {
    transition: {
      staggerChildren: 0.3,
      delayChildren: 0.4,
    },
  },
};

const heroItem: Variants = {
  hidden: { opacity: 0, y: 30 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 1.2, ease: [0.22, 1, 0.36, 1] },
  },
};

const container: Variants = {
  hidden: {},
  visible: {
    transition: {
      staggerChildren: 0.15,
      delayChildren: 0.1,
    },
  },
};

const item: Variants = {
  hidden: { opacity: 0, y: 40 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 1, ease: [0.22, 1, 0.36, 1] },
  },
};

const fadeUp: Variants = {
  hidden: { opacity: 0, y: 40 },
  visible: {
    opacity: 1,
    y: 0,
    transition: { duration: 1, ease: [0.22, 1, 0.36, 1] },
  },
};

// SEO note: this page maps to Starfii's real "Enterprise AI/ML, Gen AI
// and LLM" service (see starfii.com/services). Primary keyword targets
// woven into the copy: "artificial intelligence services", "machine
// learning development", "predictive AI models", "computer vision
// solutions", "NLP & conversational AI", "MLOps & model monitoring",
// "responsible AI", "AI consulting for enterprises".

const useCases = [
  { name: "Predictive Maintenance", detail: "AI-driven failure prediction models for manufacturing and industrial equipment that cut unplanned downtime." },
  { name: "Fraud & Risk Scoring", detail: "Real-time anomaly detection and risk-scoring models for financial services and insurance transactions." },
  { name: "Demand Forecasting", detail: "Machine learning-based inventory and supply-chain forecasting from historical and market signals." },
  { name: "Clinical Decision Support", detail: "Diagnostic-assist and triage AI models trained on structured healthcare data for faster, more accurate decisions." },
  { name: "Document Intelligence", detail: "AI-powered extraction and classification across invoices, contracts, and forms, reducing manual data entry." },
  { name: "Recommendation Engines", detail: "Personalization and recommendation AI models for e-commerce and content platforms." },
];

const capabilities = [
  { icon: BrainCircuit, title: "AI Strategy & Readiness Consulting", desc: "We assess data maturity, infrastructure, and the business case before recommending build, buy, or fine-tune, not the other way around." },
  { icon: Sparkles, title: "Custom Machine Learning Model Development", desc: "Custom ML models built and validated against your data, classification, forecasting, ranking, and recommendation systems." },
  { icon: Eye, title: "Computer Vision Solutions", desc: "Object detection, defect inspection, and image classification pipelines deployed at production throughput." },
  { icon: MessagesSquare, title: "NLP & Conversational AI", desc: "Entity extraction, document classification, and intent-driven chat and voice interfaces trained on domain language." },
  { icon: LineChart, title: "MLOps & AI Model Monitoring", desc: "CI/CD pipelines for machine learning models, with drift detection and retraining triggers so accuracy holds up after deployment." },
  { icon: ShieldCheck, title: "Responsible AI & Governance", desc: "Bias testing, explainability reporting, and human-in-the-loop review built into the AI model lifecycle, not bolted on after audit." },
];

const engagementSteps = [
  { title: "Discover", desc: "We identify the highest-value AI use case where data quality and business impact both clear the bar." },
  { title: "Prototype", desc: "A working machine learning model against real data, benchmarked for accuracy before any production commitment." },
  { title: "Productionize", desc: "The AI model is hardened, wrapped in APIs, and integrated into the systems your teams actually use." },
  { title: "Monitor & Retrain", desc: "Ongoing tracking of model drift and accuracy, with scheduled retraining as the underlying data shifts." },
];

export default function ArtificialIntelligencePage() {
  return (
    <main className="bg-white">
      {/* ===================== HERO — animates once, right after page load ===================== */}
      <section className={`${T.inkBg} relative overflow-hidden pt-[104px] pb-20 sm:pt-[118px]`}>
        <div
          className="pointer-events-none absolute inset-y-0 right-0 w-[45%]"
          style={{
            background:
              "radial-gradient(60% 80% at 100% 40%, rgba(59,47,224,0.35) 0%, rgba(143,168,255,0.18) 40%, rgba(12,14,42,0) 72%)",
          }}
        />
        <motion.div
          variants={heroContainer}
          initial="hidden"
          animate="visible"
          className={`relative grid grid-cols-1 items-center gap-14 lg:grid-cols-[1.05fr_0.95fr] ${ALIGN}`}
        >
          <div>
            <motion.nav variants={heroItem} aria-label="Breadcrumb" className="flex items-center gap-2 text-[13px] font-medium text-white/50">
              <a href="/" className="transition-colors hover:text-white/80">Home</a>
              <ChevronRight size={13} />
              <a href="/services" className="transition-colors hover:text-white/80">Services</a>
              <ChevronRight size={13} />
              <span className="text-white/75">Artificial Intelligence</span>
            </motion.nav>
            <motion.h1 variants={heroItem} className="mt-6 max-w-xl text-[40px] font-bold leading-[1.1] text-white sm:text-[52px]">
              Artificial Intelligence Services Built for{" "}
              <span className="text-[#8FA8FF]">Your Data</span>, Not a Generic Benchmark
            </motion.h1>
            <motion.p variants={heroItem} className="mt-6 max-w-lg text-[17px] leading-relaxed text-white/70">
              From AI strategy to production, we build and operate machine
              learning systems, predictive models, computer vision, NLP,
              and recommendation engines, validated against your data
              before they touch a live decision.
            </motion.p>
            <motion.div variants={heroItem} className="mt-9 flex flex-wrap items-center gap-4">
              <a href="#talk-to-us" className={`inline-flex h-12 items-center gap-2 rounded-md px-6 text-[15px] font-semibold text-white transition-colors duration-150 ${T.primaryBg} ${T.primaryHoverBg}`}>
                Talk to us
                <ArrowUpRight size={16} />
              </a>
              <a href="#use-cases" className="inline-flex h-12 items-center gap-2 rounded-md border border-white/20 px-6 text-[15px] font-semibold text-white transition-colors duration-150 hover:bg-white/10">
                See AI use cases
              </a>
            </motion.div>
          </div>

          <motion.div variants={heroItem} className="hidden lg:block">
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-7 backdrop-blur-sm">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-white/50">
                AI model types we build
              </p>
              <div className="mt-5 grid grid-cols-2 gap-3">
                {["Predictive AI", "Computer Vision", "NLP", "Recommendation"].map((name) => (
                  <div key={name} className="rounded-lg border border-white/10 bg-white/[0.03] px-4 py-3 text-[14px] font-medium text-white/85">
                    {name}
                  </div>
                ))}
              </div>
              <div className="mt-6 border-t border-white/10 pt-6">
                <p className="text-[28px] font-bold text-white">6</p>
                <p className="mt-1 text-[13px] leading-relaxed text-white/55">
                  Core artificial intelligence capability areas, from strategy through MLOps
                </p>
              </div>
            </div>
          </motion.div>
        </motion.div>
      </section>

      {/* ===================== OVERVIEW — animates on scroll into view ===================== */}
      <motion.section
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.15 }}
        variants={fadeUp}
        className={`${ALIGN} py-20`}
      >
        <div className="grid grid-cols-1 gap-14 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            <h2 className={`text-[28px] font-bold ${T.ink}`}>
              We collaborate on the problem before the AI model
            </h2>
            <p className={`mt-5 text-[15.5px] leading-relaxed ${T.muted}`}>
              Most artificial intelligence initiatives stall because the
              problem was never framed in a way a model could solve. We
              start with your business problem, identify whether it's a
              classification, forecasting, or generative problem, and only
              then choose the right machine learning model family and
              architecture.
            </p>
            <motion.ul
              initial="hidden"
              whileInView="visible"
              viewport={{ once: true, amount: 0.3 }}
              variants={container}
              className="mt-8 space-y-4"
            >
              {[
                "Data scientists and ML engineers embedded with your domain experts",
                "AI model choice driven by the problem, not defaulted to the newest architecture",
                "Every machine learning model benchmarked against a defined accuracy and latency target",
                "AI deployment designed for monitoring and retraining from day one",
              ].map((point) => (
                <motion.li key={point} variants={item} className="flex items-start gap-3">
                  <CheckCircle2 size={18} className={`mt-0.5 shrink-0 ${T.primary}`} />
                  <span className={`text-[14.5px] leading-relaxed ${T.ink}`}>{point}</span>
                </motion.li>
              ))}
            </motion.ul>
          </div>

          <div className={`rounded-2xl border ${T.border} ${T.panelBg} p-8`}>
            <p className={`text-[11px] font-semibold uppercase tracking-[0.14em] ${T.primary}`}>
              How this differs from Generative AI
            </p>
            <p className={`mt-4 text-[14.5px] leading-relaxed ${T.ink}`}>
              This practice covers predictive and applied machine learning,
              forecasting, computer vision, fraud detection, and
              recommendation systems built on your structured and
              unstructured data.
            </p>
            <p className={`mt-4 text-[14.5px] leading-relaxed ${T.muted}`}>
              For large language model (LLM) and generative AI use cases,
              copilots, content generation, retrieval-augmented assistants,
              see our dedicated Generative AI offering.
            </p>
            <a href="/services" className={`mt-5 inline-flex items-center gap-1.5 text-[14px] font-semibold ${T.primary}`}>
              View Generative AI
              <ArrowUpRight size={15} />
            </a>
          </div>
        </div>
      </motion.section>

      {/* ===================== CAPABILITIES — cards stagger in on scroll ===================== */}
      <section className={`${T.panelBg} border-y ${T.border}`}>
        <div className={`${ALIGN} py-20`}>
          <motion.h2
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.3 }}
            variants={fadeUp}
            className={`text-[28px] font-bold ${T.ink}`}
          >
            What we deliver
          </motion.h2>
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.1 }}
            variants={container}
            className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3"
          >
            {capabilities.map(({ icon: Icon, title, desc }) => (
              <motion.div key={title} variants={item} className={`rounded-xl border ${T.border} bg-white p-6`}>
                <div className={`flex h-10 w-10 items-center justify-center rounded-md ${T.primaryBg}`}>
                  <Icon size={18} className="text-white" />
                </div>
                <h3 className={`mt-4 text-[16.5px] font-semibold ${T.ink}`}>{title}</h3>
                <p className={`mt-2.5 text-[14px] leading-relaxed ${T.muted}`}>{desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      </section>

      {/* ===================== USE CASES — cards stagger in on scroll ===================== */}
      <section id="use-cases" className={`${ALIGN} py-20`}>
        <motion.h2
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.3 }}
          variants={fadeUp}
          className={`text-[28px] font-bold ${T.ink}`}
        >
          AI & machine learning use cases we've built
        </motion.h2>
        <motion.p
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.3 }}
          variants={fadeUp}
          className={`mt-3 max-w-2xl text-[15px] leading-relaxed ${T.muted}`}
        >
          Applied artificial intelligence across manufacturing, financial services, healthcare, and retail.
        </motion.p>
        <motion.div
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, amount: 0.1 }}
          variants={container}
          className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3"
        >
          {useCases.map((u) => (
            <motion.div key={u.name} variants={item} className={`border-l-2 ${T.border} pl-5`}>
              <h4 className={`text-[16px] font-semibold ${T.ink}`}>{u.name}</h4>
              <p className={`mt-1.5 text-[13.5px] leading-relaxed ${T.muted}`}>{u.detail}</p>
            </motion.div>
          ))}
        </motion.div>
      </section>

      {/* ===================== ENGAGEMENT MODEL — steps stagger in on scroll ===================== */}
      <section className={T.inkBg}>
        <div className={`${ALIGN} py-20`}>
          <motion.h2
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.3 }}
            variants={fadeUp}
            className="text-[28px] font-bold text-white"
          >
            How an AI engagement runs
          </motion.h2>
          <motion.div
            initial="hidden"
            whileInView="visible"
            viewport={{ once: true, amount: 0.1 }}
            variants={container}
            className="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4"
          >
            {engagementSteps.map((step, i) => (
              <motion.div key={step.title} variants={item} className="border-t border-white/15 pt-5">
                <p className="text-[13px] font-semibold text-white/50">
                  {String(i + 1).padStart(2, "0")}
                </p>
                <h4 className="mt-2 text-[17px] font-semibold text-white">{step.title}</h4>
                <p className="mt-2 text-[13.5px] leading-relaxed text-white/65">{step.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
      </section>

      {/* ===================== CTA — animates on scroll into view ===================== */}
      <motion.section
        initial="hidden"
        whileInView="visible"
        viewport={{ once: true, amount: 0.3 }}
        variants={fadeUp}
        id="talk-to-us"
        className={`${ALIGN} py-20`}
      >
        <div className={`flex flex-col items-start justify-between gap-6 rounded-2xl border ${T.border} ${T.panelBg} p-10 sm:flex-row sm:items-center`}>
          <div>
            <h3 className={`text-[24px] font-bold ${T.ink}`}>
              Have an AI use case you're not sure is "AI-ready"?
            </h3>
            <p className={`mt-2 text-[14.5px] ${T.muted}`}>
              Bring us the data and the decision it should improve, we'll tell you honestly if a machine learning model is worth building.
            </p>
          </div>
          <a href="/contact" className={`inline-flex h-12 shrink-0 items-center gap-2 rounded-md px-6 text-[15px] font-semibold text-white transition-colors duration-150 ${T.primaryBg} ${T.primaryHoverBg}`}>
            Talk to us
            <ArrowUpRight size={16} />
          </a>
        </div>
      </motion.section>
    </main>
  );
}

/*
  SEO checklist for this page (apply outside this component, since
  it's a client component and can't export metadata):

  1. Page <title>: "Artificial Intelligence Services | Machine Learning
     & AI Consulting | Starfii Technology" (under 60 chars).
  2. Meta description: "Starfii builds and operates AI and machine
     learning systems, predictive models, computer vision, NLP, and
     MLOps, validated against your data before going live."
  3. One <h1> per page (done), with target keywords ("artificial
     intelligence services", "machine learning development") in the H1,
     first paragraph, and multiple H2s/H3s.
  4. Internal link to the Generative AI page kept intact, update the
     href from "#" to the real route once that page exists.
*/