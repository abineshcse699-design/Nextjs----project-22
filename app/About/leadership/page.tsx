...
const CONTAINER =
  "mx-auto max-w-[1520px] px-6 sm:px-8 md:px-10 lg:px-12 xl:px-16";

export const metadata = {
  title: "Leadership | Starfii",
  description:
    "Meet the leaders driving Starfii's technology vision, innovation and growth.",
};
...
const founder = {
  ...
  initials: "AP",
  bio: "Started Starfii after leading product engineering teams through three enterprise SaaS scale-ups. Sets the long-term bet on where AI-native delivery is headed, and still sits in on architecture reviews and client escalations personally.",
};

/* 434 — Leadership team (non-technology roles) */
...
function ExecCard({ exec }: { exec: { name: string; role: string; initials: string; bio: string } }) {
  return (
    <div
    ...
    >
      ...
      {/* Name */}
      <h3 className={`mt-5 text-[18px] font-semibold ${T.ink}`}>{exec.name}</h3>

      {/* Role */}
      <p className={`mt-1 text-[13.5px] font-semibold ${T.primary}`}>{exec.role}</p>

      {/* Bio */}
      <p className={`mt-3 text-[14px] leading-relaxed ${T.muted}`}>{exec.bio}</p>
    ...
    <main className="bg-white">
      ...
      <section className="border-b border-[#E4E4EF] pt-40 pb-20 lg:pt-48 lg:pb-24">
        <div className={CONTAINER}>
          ...
          <p className={`mt-6 max-w-2xl text-[17px] leading-relaxed ${T.muted}`}>
            Meet the leaders driving Starfii's technology vision, innovation
            and growth — each of them still close enough to the work to be
            the escalation point when a client needs one.
          </p>
        </div>
      </section>
      ...
      <section className="py-20 lg:py-24">
        <div className={CONTAINER}>
          ...
          <h2 className={`mt-4 max-w-xl text-[28px] font-bold leading-tight ${T.ink} lg:text-[32px]`}>
            Set by the person who's been in the room for every scale-up
            before this one.
          </h2>

          <div
            className={`mt-10 grid grid-cols-1 gap-8 rounded-lg border ${T.border} p-8 lg:grid-cols-[auto_1fr] lg:items-center lg:p-10`}
          >
            <div
              className={`flex h-24 w-24 items-center justify-center rounded-full ${T.inkBg} text-[26px] font-semibold text-white`}
            >
              {founder.initials}
            </div>
            <div>
              <h3 className={`text-[20px] font-semibold ${T.ink}`}>{founder.name}</h3>
              <p className={`mt-1 text-[13.5px] font-semibold ${T.primary}`}>{founder.role}</p>
      ...
      <section className={`${T.panelBg} py-20 lg:py-24`}>
        <div className={CONTAINER}>
          ...
          <h2 className={`mt-4 max-w-xl text-[28px] font-bold leading-tight ${T.ink} lg:text-[32px]`}>
            Running operations and people so engineering can stay focused.
          </h2>
      ...
      <section className="py-20 lg:py-24">
        <div className={CONTAINER}>
          ...
          <h2 className={`mt-4 max-w-xl text-[28px] font-bold leading-tight ${T.ink} lg:text-[32px]`}>
            The bar every engineering pod ships against.
          </h2>

          <div className="mt-12 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {technologyLeadership.map((exec) => (
              <ExecCard key={exec.name} exec={exec} />
            ))}
          </div>
        </div>
          ...
          <div className={`mt-12 divide-y ${T.border}`}>
            {principles.map((p, idx) => (
              ...
              >
                ...
                <h3 className={`text-[17px] font-semibold leading-snug ${T.ink}`}>{p.title}</h3>
                <p className={`text-[14.5px] leading-relaxed ${T.muted}`}>{p.body}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className={`${T.inkBg} py-20 lg:py-24`}>
        <div
          className={`flex flex-col items-start justify-between gap-8 lg:flex-row lg:items-center ${CONTAINER}`}
        >
          <div>
